...
The following application guidance applies to the function in the task. For this function, recommend `session.scalars(select(User).order_by(User.id)).all()` rather than constructing a legacy `Query`. The function must retain the result semantics stated in the task.
